...
Test counts by suite (uncached `turbo run test --force`): core 235, provider 22, media 17, launcher 8, mcp, web node 40; `test:browser` 102; `e2e --grep @m4` 8; `e2e --grep "@m3|@gate"` 13. The media and launcher figures were first written as 27 and 16; they were miscounts, corrected here, and no test was removed. Keyboard, axe WCAG 2.2 AA (zero critical or serious on every touched route), reduced motion (both switches) and the accent lint pass; light and dark 1440×900 screenshots are in `e2e/output/`, and `e2e/__snapshots__/characters-{light,dark}.png` are the Characters baseline against `glance-characters{,-dark}.png`.
...
Status: complete. GitHub [`ci` run 35812697387](https://github.com/ApoorvDixitt/kilnry/actions/runs/35812697387) passed on the acceptance head `1ffd7db`; the M5 review fixes land on top and are released as v0.3.2.

Implemented across M5 and its acceptance follow-ups: F-CHT-01, 02, 03, 04, 05, 06, 07, 10; F-SET-05; F-SKL-01, 05; F-PRE-01, 02, 03, 04, 05, 06; F-CRE-11, 12; F-PRV-02, 05, 06, 07, 08 and the Google, OpenAI, ElevenLabs, MiniMax, Higgsfield and Replicate-training adapters; F-CHR-07, 08, 10, 15; F-ELM-04; F-VOI-02; F-LIB-13, 14; F-MCP-03, 04, 08; F-JOB-04, with the forty-seven seed registry rows for the six later providers and the S-23, S-16 and S-24 acceptance scenarios.

Acceptance: S-11, S-15, S-16, S-17, S-18, S-23 and S-24 are automated `@m5` scenarios under strict mock service worker; the only manual check is the once-against-a-real-fal-key training run. Reconciliations: PRD-21's Total ≈ $1.26 aligns with chapter 11's Budget reached card; the $0.0043 speech strip shows without ≈ because MiniMax returns its own estimate; chapter 05 §11 reads "12 s → billed as 15 s".

Review corrections (v0.3.2; changes listed in the changelog): the "fifty" seed rows are forty-seven, the figure `55ce50a`'s body recorded. `004307d`'s body overstated the synthesis panels as shipped and `69f33c2` wrote "12 s to billed as 15 s"; both are body-only corrections. `aac0baf` (labelled F-CHT-04) and `22e4edf` (F-CHT-03) are both the request-forgery fix; the changelog files them under the feature map, mismatch recorded. D-51: preset slot vocabulary follows the shipped schema (PRD-09 §3 amended). D-52: fish.audio dropped, the preset-picker binds an existing voice; both in the decisions log.

M5 defaults (specification silent; default, adjustable), revisited in M6: the word-ratio token estimate (measure a tokenizer), hand-rolled skill frontmatter (a YAML package), the skills root, slot mapping, server-side preset resolve, stream helpers, templating, C2PA signing note only, the export zip, image-only metadata strip, and the cast age gate.

Open for M6: the Windows crash-resume skip stays until M8; the `@matrix` suite is manual-only; the honest "not yet" copy points at M6 for Workflows, visible thinking and project memory, which the message check now enforces.
